...
locals {
  ...
  ami_kubernetes_version = local.need_ami_id ? regex("^(\\d+\\.\\d+)", coalesce(var.ami_release_version, var.kubernetes_version))[0] : ""
  ...
  ami_id = local.need_ami_id ? data.aws_ami.selected[0].image_id : var.ami_image_id
}
